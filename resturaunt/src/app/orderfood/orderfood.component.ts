...
export class OrderfoodComponent {
  ...
  placeOrder(): void {
    ...
    const orderDetails = {
      ...
      user: this.user, // The logged-in user
      orderDate: new Date(), // Current date
      finalPrice: this.totalPrice, // Total price calculated in cart
      paymentMethod: 'Cash', // Or allow user to select payment method
    };

    // Prepare orders list from the cart
    const orders = this.cart.map(item => {
      return {
        id: 0, // ID will be generated by the backend
        user: this.user, // You may want to ensure this is set correctly
        food: item.food,
        quantity: item.quantity,
        totalPrice: item.food.price * item.quantity,
        status: 'PENDING', // Default status
        notes: '', // Optional notes
        admin: null, // Admin can be assigned later
        staff: null // Staff can be assigned later
      };
    });

    // Call order service to place the order with order details and orders
